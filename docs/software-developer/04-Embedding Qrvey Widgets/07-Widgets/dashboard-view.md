...
| **pageId** | `String`, ID of one page to visualize it: all auth process is still required if the configuration exists. | **Optional** |
| **userFilters** | `Array<Object>`, Collection of custom filters that the system will apply to the visualized data. For more information, please see [Working With Filters in Embedded Scenarios](../Embedding%20Qrvey%20Widgets/filters-embedded-scenarios). | **Optional** |
...
| **featurePermission** | `Object`, Configures which features are available in the widget. Presently it only supports showing and hiding of the pages bar, where the pages appear as tabs at the bottom of the widget. | **Optional** |
| **subscriptionSettings** | `Object`, Configures which features are available in the widget. Presently it only supports showing and hiding of the pages bar, where the pages appear as tabs at the bottom of the widget. | **Optional** |
For more details, please see [The Feature Permission Object](#the-feature-permission-object) below. | **Optional** |
...
| **hideNavigationTab** | `Boolean`, Hides the entry method to the navigation tab in the top bar of the widget. | **Optional** |
| **userManagement** | `Object`, Defines user management-related features that can be hidden. | **Optional** |
| **hideUserManagementTab** | `Boolean`, Hides the entry method to the user management tab in the top bar of the widget. | **Optional** |
| **pagesAndApplication** | `Object`, Defines pages and application-related features that can be hidden. | **Optional** |
| **hidePublishAppButton** | `Boolean`, Hides the “Unpublish”/“Publish Application” button. | **Optional** |
| **hidePublishPageButton** | `Boolean`, Hides the “Publish Page” button. | **Optional** |
| **hideCopyPageLink** | `Boolean`, Hides the UI interfaces where the user can get the link of a page. | **Optional** |
| **hideLaunchButton** | `Boolean`, Hides the button to access the page view. | **Optional** |
| **hideCreateManagePages** | `Boolean`, Hides all options to create pages. | **Optional** |
| **hidePageStatus** | `Boolean`, Hides the “Status” text. | **Optional** |
| **hidePagesBar** | `Boolean`, Hides the bottom bar for pages. | **Optional** |
| **canvas** | `Object`, Defines canvas-related features that can be hidden. | **Optional** |
| **hideManageCanvas** | `Boolean`, Hides the following options: Grid, Responsive View, Discard Changes. | **Optional** |
...
| **downloads.hideSchedule** | `Boolean`, If true, hides the scheduling export option in the export modal. | **Optional** |
...
| **emails** | `Object Array`, Email addresses to deliver the exports to. | **Optional** | 
...
| **Property** | **Description** | **Required** |
| --- | --- | --- |
| **enabled** | `Boolean`, If false, disables personalization for end users and all of the properties in the personalization object, other than the **fit_panel_button** property will be omitted. Defaults to true. | **Optional** |
